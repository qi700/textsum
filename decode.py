...
if __name__ == '__main__':
    ...
    if opt.task == "validate":
        ...
        for f in saved_models:
            eval_processor = BeamSearch(config.save_model_path+'/'+f)
            eval_processor.decode(f)
    elif opt.task == "test":
        eval_processor = BeamSearch(config.save_model_path+'/'+opt.load_model)
        eval_processor.decode(opt.load_model)
    else:
